...
# The detect_gpu() macro aims to detect GPUs available in the build machine
...
# - DETECT_GPU_AVAILABLE - TRUE if any GPUs were detected, otherwise FALSE
# - DETECT_GPU_COUNT     - # of GPUs detected
# - DETECT_GPU_INFO      - list of information strings of the detected GPUs
#
...
macro(check_num_gpu_info NGPU GPU_INFO)
    ...
    if (NOT NGPU EQUAL _len)
        list(APPEND ${DETECT_GPU_INFO} "NOTE: information about some GPU(s) missing!")
    endif()
endmacro()

macro(DETECTGPU)

    if (NOT DEFINED DETECT_GPU_COUNT OR NOT DEFINED DETECT_GPU_INFO)
        ...
        if (NOT DEFINED DETECT_GPU_COUNT_NVIDIA_SMI)
        ...
        if (UNIX AND NOT (APPLE OR CYGWIN))
            ...
            if (NOT DEFINED DETECT_GPU_COUNT_PROC AND DETECT_GPU_COUNT EQUAL 0)
                set(DETECT_GPU_COUNT_PROC 0)
                file(GLOB _proc_nv_gpu_info "/proc/driver/nvidia/gpus/*/information")
                foreach (_file ${_proc_nv_gpu_info})
                    math(EXPR DETECT_GPU_COUNT_PROC "${DETECT_GPU_COUNT_PROC}+1")
                    ...
                    if (NOT _gpu_id STREQUAL "" AND NOT _gpu_name STREQUAL "")
                        list(APPEND DETECT_GPU_INFO "GPU ${_gpu_id}: ${_gpu_name}")
                    endif()
                endforeach()

                check_num_gpu_info(${DETECT_GPU_COUNT_PROC} DETECT_GPU_INFO)
                set(DETECT_GPU_COUNT ${DETECT_GPU_COUNT_PROC})
            ...
            if (NOT DEFINED DETECT_GPU_COUNT_LSPCI AND DETECT_GPU_COUNT EQUAL 0)
                set(DETECT_GPU_COUNT_LSPCI 0)
                ...
                if (_lspci_ret EQUAL 0)
                    ...
                    foreach(_line ${_lspci_out})
                        ...
                        if (_line_upper MATCHES ".*VGA.*NVIDIA.*" OR _line_upper MATCHES ".*3D.*NVIDIA.*")
                            math(EXPR DETECT_GPU_COUNT_LSPCI "${DETECT_GPU_COUNT_LSPCI}+1")
                            ...
                            if (NOT _gpu_name EQUAL "")
                                list(APPEND DETECT_GPU_INFO "${_gpu_name}")
                            endif()
                        endif()
                    endforeach()

                    check_num_gpu_info(${DETECT_GPU_COUNT_LSPCI} DETECT_GPU_INFO)
                    set(DETECT_GPU_COUNT ${DETECT_GPU_COUNT_LSPCI})
                endif()
        ...
        if (DETECT_GPU_COUNT GREATER 0)
            set(DETECT_GPU_AVAILABLE TRUE)
        else()
            set(DETECT_GPU_AVAILABLE FALSE)
        endif()
        set(DETECT_GPU_AVAILABLE ${DETECT_GPU_AVAILABLE} CACHE BOOL "Whether any NVIDIA GPU was detected" FORCE)

        set(DETECT_GPU_COUNT ${DETECT_GPU_COUNT}
            CACHE STRING "Number of NVIDIA GPUs detected")
        set(DETECT_GPU_INFO ${DETECT_GPU_INFO}
            CACHE STRING "basic information on the detected NVIDIA GPUs")

        set(DETECT_GPU_COUNT_NVIDIA_SMI ${DETECT_GPU_COUNT_NVIDIA_SMI}
            CACHE INTERNAL "Number of NVIDIA GPUs detected using nvidia-smi")
        set(DETECT_GPU_COUNT_PROC ${DETECT_GPU_COUNT_PROC}
            CACHE INTERNAL "Number of NVIDIA GPUs detected in /proc/driver/nvidia/gpus")
        set(DETECT_GPU_COUNT_LSPCI ${DETECT_GPU_COUNT_LSPCI}
            CACHE INTERNAL "Number of NVIDIA GPUs detected using lspci")

        mark_as_advanced(DETECT_GPU_AVAILABLE
                         DETECT_GPU_COUNT
                         DETECT_GPU_INFO)

        if (DETECT_GPU_AVAILABLE)
            message(STATUS "Number of NVIDIA GPUs detected: ${DETECT_GPU_COUNT} ")
        else()
            message(STATUS "Could not detect NVIDIA GPUs")
        endif()

    endif (NOT DEFINED DETECT_GPU_COUNT OR NOT DEFINED DETECT_GPU_INFO)
endmacro(DETECTGPU)
